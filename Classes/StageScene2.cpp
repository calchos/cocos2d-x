...
#include "StageScene2.h" // StageScene2.h読み込み
#include "StageConversation3.hpp" // StageConversation3.hpp読み込み
#include "StageConversation4.hpp" // StageConversation4.hpp読み込み
...
bool StageScene2::init()
{
    ...
    listener->onTouchBegan = CC_CALLBACK_2(StageScene2::onTouchBegan, this);// タップした瞬間に1回だけ処理を実行
    listener->onTouchEnded = CC_CALLBACK_2(StageScene2::onTouchEnded, this);// タップを離した瞬間に１回だけ実行
    ...
    // 敵出現テキスト
    auto _text1 = Label::createWithSystemFont("マンドラ先輩があらわれた！▼", "fonts/ヒラギノ明朝 ProN W6.ttc",18);
    ...
    // ステージ2背景表示
    auto background2 = Sprite::create("res/dream_world_forest.png");
...
bool StageScene2::onTouchBegan(Touch* pTouch, Event* pEvent){
    
    // 戦闘に勝利したら会話シーン4の画面に遷移する
    if(_state == TYPE_WIN){
        Director::getInstance()->replaceScene(TransitionFade::create(1.0
                                                                     , StageConversation4::createScene()
                                                                     , Color3B::WHITE));
    }
    // 戦闘に負けたら会話シーン3の画面に戻る
    if(_state == TYPE_LOSE){
        Director::getInstance()->replaceScene(TransitionFade::create(2.0
                                                                     , StageConversation3::createScene()
                                                                     , Color3B::BLACK));
    }
    // キャラクタータグ呼び出し
    ...
    // 2.0秒後にintervalDeleteActionメソッドを呼ぶ
    this->scheduleOnce(schedule_selector(StageScene2::intervalDeleteAction),2.0f);
...
void StageScene2::onTouchEnded(Touch* pTouch, Event* pEvent){
    ...
    if(_state == TYPE_ENEMY_TURN){
        ...
        if(!_enemy1->_doAttack){
            ...
            this->runAction(Sequence::create(DelayTime::create(2),CallFunc::create([this](){
                ...
                auto _text3 = Label::createWithSystemFont("マンドラ先輩の絶死の叫び。\nクリスタルがマンドラ先輩に力を与える▼", "fonts/ヒラギノ明朝 ProN W6.ttc",18);
            ...
            // プレイヤーのトータルクリスタル数値が200より大きい時
            /*
             if(_totalCrystalPower >= 200){
             
             // 通常合体技発動！
             auto _text7 = Label::createWithSystemFont("クリスタルの力により合体技：龍光劔発動！▼", "fonts/ヒラギノ明朝 ProN W3.ttc",18);
             _text7->setPosition(Point(visibleSize.width / 2 + origin.x, visibleSize.height / 2 + origin.y - 180));
             _text7->setTag(10);//タグ付け
    ...
    // 勝利イベント
        ...
        if(!_gameEndEvent){
            ...
            this->runAction(Sequence::create(DelayTime::create(4),CallFunc::create([this](){
                ...
                // 勝利メッセージ
                auto _text6 = Label::createWithSystemFont("マンドラ先輩をやっつけた！▼", "fonts/ヒラギノ明朝 ProN W6.ttc",18);
...
// 一定時間後に削除実行するアクション
void StageScene2::intervalDeleteAction(float delta){
...
// 敵のトータルダメージ計算処理
int StageScene2::enemyAttack(){
    
    // 乱数初期化
    srand((unsigned int)time(NULL));
    int m = 0;// 乱数用変数の初期値
    _enemyCrystalPower = rand()%(m+299);
...
// 先制防御
int StageScene2::playerPredefense(){
...
// 敵キャラクターの先制攻撃処理
int StageScene2::enemyPreemptiveAttack(){
    ...
    if(r == 1){
        _preemptive_attack = true; // 先制攻撃フラグをtrueにする
        auto _text5 = Label::createWithSystemFont("マンドラ先輩の絶死の叫び！", "fonts/ヒラギノ明朝 ProN W6.ttc", 48);
